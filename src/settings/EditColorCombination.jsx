...
function EditColorCombination({ index, card, setCards, modelDetails, cards }) {
    const [intColors, setIntColors] = useState("");

    const [extColors, setExtColors] = useState("");
    useEffect(() => {
        setIntColors(card.intColor);
        setExtColors(card.extColor);
    }, []);
    ...
    const removeColors = (card) => {
        let Newcards = cards.filter((cards) => cards != card);
        setCards(Newcards);
    };
    return (
        <>
            <div className="lg:grid grid-cols-3 gap-5 my-3">
                ...
                <div className="lg:flex items-center-safe justify-between">
                    ...
                    <Trash2 className="w-4 text-red-600 cursor-pointer" onClick={() => removeColors(card)} />
                </div>
            </div>
        </>
    );
...
export default EditColorCombination;
