...
function AddTrim({modelDetails,setModelDetails,cards,setCards}) {
    ...
    return (
        <>
            ...
            <div className="relative">
                <div className="lg:grid grid-cols-2 gap-5 my-6">
                    <div className="">
                        ...
                        <input type="number" value={modelDetails.unitPrice} className="outline  outline-gray-300 rounded-sm py-1 lg:w-full px-2" onChange={(e)=>setModelDetails({...modelDetails,unitPrice:e.target.value})}/>
                    </div>

                    <div>
                        <p className="text-sm font-medium">Retail Price</p>
                        <input type="number" value={modelDetails.retailPrice} className="outline  outline-gray-300 rounded-sm py-1 lg:w-full px-2" onChange={(e)=>setModelDetails({...modelDetails,retailPrice:e.target.value})}/>
                    </div>
                </div>
            ...
            {cards.map(( index) => (
                <Card className="bg-gray-100 my-2 p-4">
                    <ColorCombination key={index} index={index} cards={cards} setCards={setCards} modelDetails={modelDetails} setModelDetails={setModelDetails}/>
                </Card>
            ))}
        </>
    );
